 ...
 * @Copyright: 2018
 */
public class CollectSqliteUpHelp {
    private static volatile CollectSqliteUpHelp _singleton;
    private Context mContext;
    private final DbQueryUtil mDbQueryUtil;
    private final SQLiteDatabase mSqLiteDatabase;

    private CollectSqliteUpHelp(Context context) {
        this.mContext = context;
        mSqLiteDatabase = createtable();
        mDbQueryUtil = DbQueryUtil.get_Instance();
    }

    public static CollectSqliteUpHelp get_Instance(Context context) {
        if (_singleton == null) {
            synchronized (CollectSqliteUpHelp.class) {
                if (_singleton == null) {
                    _singleton = new CollectSqliteUpHelp(context);
                }
            }
        }
        return _singleton;
